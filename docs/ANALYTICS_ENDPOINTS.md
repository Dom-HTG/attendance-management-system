...
This document provides complete reference for the analytics system, including overview, feature summary, and detailed API endpoint documentation.

## Overview

The analytics system provides comprehensive insights for students, lecturers, and administrators through 25+ REST endpoints and natural language recommendations. It covers student performance metrics, lecturer course analytics, university-wide dashboards, temporal patterns, anomaly detection, predictive models, and peer benchmarking.
...
- **Predictive Analytics**: Attendance forecasting, risk indicators, dropout prevention
- **Anomaly Detection**: Duplicate check-ins, suspected fraud, unusual patterns
- **Benchmarking**: Peer comparisons, percentile ranking, historical analysis
- **Visualization Support**: Chart data for frontend rendering (line, bar, pie, heatmap)
...
### Architecture
- **Domain Layer**: 75+ response DTOs covering all analytics sections
- **Repository Layer**: 30+ optimized SQL queries with strategic database indexes
- **Service Layer**: Business logic with natural language insight generation
- **HTTP Handler Layer**: 25+ REST endpoints with role-based authorization
- **Database**: PostgreSQL with 8 performance indexes

### Key Metrics
- **Endpoints**: 25+ REST routes with full CRUD operations
- **Data Models**: 75+ domain objects for type-safe responses
- **SQL Queries**: 30+ optimized queries with proper aggregations
- **Database Indexes**: 8 strategic indexes for <500ms performance
- **Authorization**: Role-based access control (student, lecturer, admin)

### Performance Targets
- Single-entity queries: <500ms
...
**Access Control**:
- Students: view own metrics only
- Lecturers/Admins: view any student's metrics
...
```json
{
  ...
  "data": {
    ...
    "recommendations": [
      {
        "action": "Continue current engagement level",
        "priority": "low",
        "expected_impact": "Maintain excellent standing",
        "timeframe": "ongoing"
      }
    ],
    "generated_at": "2025-11-29T15:30:00Z"
...
**Response** (200 OK): Similar to student insights, with lecturer-specific recommendations
...
```
GET /api/analytics/temporal?start_date={start}&end_date={end}&granularity={granularity}
Authorization: Bearer <token>
```
...
```
GET /api/analytics/predictions/student/{student_id}
Authorization: Bearer <token>
```
...
```
GET /api/analytics/predictions/course/{course_code}
Authorization: Bearer <token>
```

**Response** (200 OK): Similar structure with course-level predictions
...
```
GET /api/analytics/benchmark?entity_type={type}&entity_id={id}
Authorization: Bearer <token>
```
...
#### Get Chart Data
```
GET /api/analytics/charts/{chart_type}?entity_type={type}&entity_id={id}
Authorization: Bearer <token>
```
...
1. Store JWT token from login response (access_token field)
2. Include token in all analytics requests: `Authorization: Bearer <token>`
3. Use chart data endpoints to render visualizations with Chart.js or similar
4. Implement polling (30s intervals) for real-time dashboard updates
5. Show at-risk flags when student/course data has `at_risk_status: true`

### For Backend Integration
- All queries optimized for <500ms response time
- Caching ready (in-memory, can migrate to Redis)
- Indexes created: run `migrations/analytics_indexes.sql` after initial setup
